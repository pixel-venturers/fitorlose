...
export const CHALLENGE_SELECT = `
  ...
  u.name AS "ownerName", u.email AS "ownerEmail", u."imageUrl" AS "ownerImageUrl"
`;
...
export function mapChallengeCard(row, viewer = null) {
  ...
  return {
    ...
    // Stored profile photo, shown only when identity is revealed (never for anonymous).
    avatarUrl: reveal ? (row.ownerImageUrl ?? null) : null,
