...
export async function getCurrentUser() {
  ...
  const created = await queryOne(
    `INSERT INTO "user" ("id", "clerkId", "email", "name", "imageUrl", "role", "updatedAt")
    ...
    [randomUUID(), clerkId, email, name, imageUrl, resolveRole(email)]
  );
 ...
 * Unlike getCurrentUser, this always refreshes name/email/imageUrl from Clerk so
 * an existing user's stored profile photo stays current after they change it.
 */
export async function syncCurrentUser() {
  ...
  const email =
    ...
    clerkUser?.emailAddresses?.[0]?.emailAddress ??
    null;
  // Nothing fresh to sync without a Clerk profile/email — fall back to JIT.
  if (!clerkUser || !email) return getCurrentUser();
