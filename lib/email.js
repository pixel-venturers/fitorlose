...
// sendMail + the user-facing builders send from a named Zoho profile
...
export async function sendAdminEmail({ subject, html, text }) {
  try {
    ...
    const { transport } = GetMailConfig("admin");
    await transport.sendMail({
      from: `FitOrLose Alerts <${process.env.ADMIN_EMAIL}>`,
...
/** Generic best-effort send from a named Zoho profile to one recipient. */
...
export async function sendRewardEmail({ to, name, challenge, amount, currency }) {
